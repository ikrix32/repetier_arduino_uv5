...
long baudrate = REP_BAUDRATE;         ///< Communication speed rate.
#if USE_ADVANCE
...
void PrintLine::moveRelativeDistanceInSteps(int32_t x,int32_t y,int32_t z,int32_t e,float feedrate,bool waitEnd,bool checkEndstop)
{
    ...
    {
        Console::printWarningFLN(PSTR("moveRelativeDistanceInSteps / queueDeltaMove returns error"));
    }
#else
...
// Set in delta move calculation
    ...
    {
        logLine();
        Console::printFLN(Messages::tDBGLimitInterval, limitInterval);
        Console::printFLN(Messages::tDBGMoveDistance, distance);
        Console::printFLN(Messages::tDBGCommandedFeedrate, Printer::feedrate);
        Console::printFLN(Messages::tDBGConstFullSpeedMoveTime, timeForMove);
    }
#endif
...
inline void PrintLine::computeMaxJunctionSpeed(PrintLine *previous,PrintLine *current)
{
    ...
    if(Printer::debugEcho())
    {
        Console::printF(PSTR("ID:"),(int)previous);
        Console::printFLN(PSTR(" MJ:"),previous->maxJunctionSpeed);
    }
...
void PrintLine::updateStepsParameter()
{
    ...
    if(Printer::debugEcho())
    {
        Console::printFLN(Messages::tDBGId,(int)this);
        Console::printF(Messages::tDBGVStartEnd,(long)vStart);
        Console::printFLN(Messages::tSlash,(long)vEnd);
        Console::printF(Messages::tDBAccelSteps,(long)accelSteps);
        Console::printF(Messages::tSlash,(long)decelSteps);
        Console::printFLN(Messages::tSlash,(long)stepsRemaining);
        Console::printF(Messages::tDBGStartEndSpeed,startSpeed,1);
        Console::printFLN(Messages::tSlash,endSpeed,1);
        Console::printFLN(Messages::tDBGFlags,flags);
        Console::printFLN(Messages::tDBGJoinFlags,joinFlags);
    }
#endif
}
...
#ifdef DEBUG_QUEUE_MOVE
    Console::printFLN(Messages::tDBGId,(int)this);
    Console::printArrayFLN(Messages::tDBGDelta,delta);
    Console::printFLN(Messages::tDBGDir,dir);
    Console::printFLN(Messages::tDBGFlags,flags);
    Console::printFLN(Messages::tDBGFullSpeed,fullSpeed);
    Console::printFLN(Messages::tDBGVMax,(int32_t)vMax);
    Console::printFLN(Messages::tDBGAcceleration,accelerationDistance2);
    Console::printFLN(Messages::tDBGAccelerationPrim,(int32_t)accelerationPrim);
    Console::printFLN(Messages::tDBGRemainingSteps,stepsRemaining);
#if USE_ADVANCE
#if ENABLE_QUADRATIC_ADVANCE
    Console::printFLN(Messages::tDBGAdvanceFull,advanceFull >> 16);
    Console::printFLN(Messages::tDBGAdvanceRate,advanceRate);
#endif
#endif
#endif // DEBUG_QUEUE_MOVE
}
...
#define RETURN_0(s) { Console::printErrorFLN(PSTR(s)); return 0; }
/*#define RETURN_0(s) { Console::print(s " "); SHOWS(temp); SHOWS(opt);\
...
//	out.println_byte_P(PSTR("Calculate delta segments:"), p->numDeltaSegments);
    ...
    for (int s = numDeltaSegments; s > 0; s--)
    {
        ...
        if (transformCartesianStepsToDeltaSteps(destinationSteps, destinationDeltaSteps))
        {
            ...
            for(i = 0; i < TOWER_ARRAY; i++)
            {
                ...
                if (delta > 0)
                {
                    ...
                    if (delta > 65535)
                        Console::printFLN(Messages::tDBGDeltaOverflow, delta);
#endif
        ...
        else
        {
            ...
            Console::printWarningF(Messages::tInvalidDeltaCoordinate);
            Console::printF(PSTR(" x:"), destinationSteps[X_AXIS]);
            Console::printF(PSTR(" y:"), destinationSteps[Y_AXIS]);
            Console::printFLN(PSTR(" z:"), destinationSteps[Z_AXIS]);
            d->dir = 0;
...
uint8_t PrintLine::queueDeltaMove(uint8_t check_endstops,uint8_t pathOptimize, uint8_t softEndstop)
{
    ...
    if (cartesianDir & XY_STEP)
    {
...
#ifdef DEBUG_SPLIT
        Console::printFLN(Messages::tDBGDeltaSeconds, seconds);
#endif
        ...
        if(segDist > Printer::maxRealSegmentLength)
        {
            ...
            Console::printFLN("StepsPerSecond:",sps);
            Console::printFLN(PSTR("New max. segment length:"),segDist);
        }
#endif
        //Console::printFLN(PSTR("Segments:"),segmentCount);
    }
    else
    {
        ...
        // the queued moves;
#ifdef DEBUG_SPLIT
        Console::printFLN(Messages::tDBGDeltaZDelta, cartesianDeltaSteps[Z_AXIS]);
#endif
...
#ifdef DEBUG_SPLIT
    Console::printFLN(Messages::tDBGDeltaSegments, segmentCount);
    Console::printFLN(Messages::tDBGDeltaNumLines, numLines);
    Console::printFLN(Messages::tDBGDeltaSegmentsPerLine, segmentsPerLine);
#endif
    ...
    for (int lineNumber = 1; lineNumber < numLines + 1; lineNumber++)
    {
        ...
        if (maxDeltaStep == 65535)
        {
            ...
            SHOWS(Printer::destinationSteps[Y_AXIS]);
            SHOWS(Printer::destinationSteps[Z_AXIS]);
            ...
            SHOW(softEndstop);
            */
            Console::printWarningFLN(PSTR("in queueDeltaMove to calculateDeltaSubSegments returns error."));

            return false;
        }

#ifdef DEBUG_SPLIT
        Console::printFLN(Messages::tDBGDeltaMaxDS, (int32_t)maxDeltaStep);
#endif
        ...
        if (virtual_axis_move == 0 && p->delta[E_AXIS] == 0)
        {
            if (numLines!=1)
            {
                Console::printErrorFLN(Messages::tDBGDeltaNoMoveinDSegment);
                /*SHOW(numLines);
                SHOW(segmentCount);
                SHOW(DELTASEGMENTS_PER_PRINTLINE);
                SHOW(segmentsPerLine);
...
#ifdef DEBUG_SPLIT
        Console::printFLN(Messages::tDBGDeltaStepsPerSegment, p->numPrimaryStepPerSegment);
        Console::printFLN(Messages::tDBGDeltaVirtualAxisSteps, p->stepsRemaining);
#endif
        p->calculateMove(axis_diff,pathOptimize);
        for (uint8_t i=0; i < E_AXIS_ARRAY; i++)
        {
...
void PrintLine::arc(float *position, float *target, float *offset, float radius, uint8_t isclockwise)
{
    ...
    //krix float linear_travel = 0; //target[axis_linear] - position[axis_linear];
...
int32_t PrintLine::bresenhamStep() // Version for delta printer
{
...
#endif
    {
        ...
        if(cur->isBlocked())   // This step is in computation - shouldn't happen
        {
            if(lastblk != (int)cur)
            {
                HAL::allowInterrupts();
                lastblk = (int)cur;
                Console::printFLN(Messages::tBLK,linesCount);
            }
            cur = NULL;
#if CPU_ARCH==ARCH_ARM
    ...
    if(doEven &&(cur->stepsRemaining <= 0 || cur->isNoMove()))   // line finished
    {
        ...
        {
            Console::printFLN(PSTR("Missed steps:"), cur->totalStepsRemaining);
            Console::printFLN(PSTR("Step/seg r:"), stepsPerSegRemaining);
            Console::printFLN(PSTR("NDS:"), (int) cur->numDeltaSegments);
            Console::printFLN(PSTR("HS:"), (int) cur->halfStep);
        }
#endif
        //HAL::forbidInterrupts();
        //deltaSegmentCount -= cur->numDeltaSegments; // should always be zero
...
int32_t PrintLine::bresenhamStep() // version for cartesian printer
{
...
#endif
    {
        ...
        if(cur->isBlocked())   // This step is in computation - shouldn't happen
        {
            /*if(lastblk!=(int)cur) // can cause output errors!
            {
                HAL::allowInterrupts();
                lastblk = (int)cur;
                Console::printFLN(Messages::tBLK,lines_count);
            }*/
            cur = NULL;
#if CPU_ARCH==ARCH_ARM
            PrintLine::nlFlag = false;
    ...
    if(doEven && (cur->stepsRemaining <= 0 || cur->isNoMove()))   // line finished
    {
        ...
        {
            Console::printF(Messages::tDBGMissedSteps,cur->totalStepsRemaining);
            Console::printFLN(Messages::tComma,cur->stepsRemaining);
        }
#endif
        removeCurrentLineForbidInterrupt();
        Printer::disableAllowedStepper();
