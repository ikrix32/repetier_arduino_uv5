...
void SdFat::errorPrint() {
  ...
  Console::printFLN(Messages::tSDErrorCode,card_.errorCode());
}
//------------------------------------------------------------------------------
...
void SdFat::errorPrint(char const* msg) {
  Console::printFLN(Messages::tError,msg);
  errorPrint();
}
//------------------------------------------------------------------------------
/** %Print msg, any SD error code.
...
void SdFat::errorPrint_P(FSTRINGPARAM(msg)) {
  Console::printF(Messages::tError);
  Console::printFLN(msg);
  errorPrint();
}
//------------------------------------------------------------------------------
/**
    ...
    while ((p = parent->getLongFilename(p, tempLongFilename, 0, NULL)))
    {
                   ...
                   if(level)
                    {
                     ...
                     Console::printF(Messages::tSlash);
                    }
                  Console::print(tempLongFilename);
                  Console::printFLN(Messages::tSlash); //End with / to mark it as directory entry, so we can see empty directories.
                }
            }
            SdBaseFile next;
            char *tmp;
        ...
        else
        {
            ...
            else
            {
                ...
                Console::print(tempLongFilename);
#if SD_EXTENDED_DIR
                Console::printF(Messages::tSpace,(long)p->fileSize);
#endif
                Console::println();
            }
        }
...
int8_t SdBaseFile::lsPrintNext(uint8_t flags, uint8_t indent) {
  ...
  for (uint8_t i = 0; i < indent; i++) Console::print(' ');
...
#ifdef GLENN_DEBUG
  Console::print("Open File:");
  Console::print((char*)dname);
  Console::println();
#endif
  vol_ = dirFile->vol_;
  dirFile->rewind();
  ...
  while ((p = dirFile->getLongFilename(p, tempLongFilename, cVFATNeeded, &wIndexPos)))
    ...
    if (!(oflag & O_CREAT) || !(oflag & O_WRITE)) {
      ...
      if (wIndexPos != 0)
        ...
        emptyFound = true;
#ifdef GLENN_DEBUG
        Console::print("Empty FAT:");
        Console::print((long)wIndexPos);
        Console::println();
#endif
        index = wIndexPos >> 5;
        }
...
#ifdef GLENN_DEBUG
    Console::print("At Index:");
    Console::print((long)index);
    Console::print("-");
    Console::print((long)bShortName);
    Console::println();
...
#ifdef GLENN_DEBUG
  Console::print("CATR:");
  Console::print(firstCluster_);
  Console::print("-");
  Console::print(p->name[0]);
  Console::println();
#endif
...
void SdBaseFile::printFatDate(uint16_t fatDate) {
  Console::print((int)FAT_YEAR(fatDate));
  Console::print('-');
...
#ifdef GLENN_DEBUG
      Console::print("Attr:");
      Console::print((long)dir->attributes);
      Console::print(" ");
      Console::print((long)dir->name[0]);
      Console::println();
#endif
...
#ifdef GLENN_DEBUG
            Console::print("Need: ");
            Console::print(cVFATNeeded);
            Console::print(" Got: ");
            Console::print(cVFATFound);
            Console::print(" ");
            Console::println();
#endif
...
#ifdef GLENN_DEBUG
  Console::print("Cur Pos:");
  Console::print(curPosition_);
  Console::println();
#endif
...
#ifdef GLENN_DEBUG
  Console::print("BC");
  Console::print((long)blockOfCluster);
  Console::print("-");
  Console::print((long)blockOffset);
  Console::print("-");
  Console::print((long)curCluster_);
  Console::print("-");
  Console::print((long)firstCluster_);
  Console::print("-");
  Console::print((long)vol_->blocksPerCluster_);
  Console::println();
#endif
...
#ifdef GLENN_DEBUG
      Console::print("Rd blk:");
      Console::print(blockNumber);
      Console::println();
#endif
...
bool SdVolume::cacheSync() {
  if (cacheStatus_ & CACHE_STATUS_DIRTY) {
#ifdef GLENN_DEBUG
    Console::print("Wr blk:");
    Console::print(cacheBlockNumber_);
    Console::println();
#endif
    if (!sdCard_->writeBlock(cacheBlockNumber_, cacheBuffer_.data)) {
      DBG_FAIL_MACRO;
