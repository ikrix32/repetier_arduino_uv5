        ...
        if(decoupleTestRequired && act->isDecoupleFullOrHold() && Printer::isPowerOn()) // Only test when powered
        {
            if(act->isDecoupleFull())
            {
                if(act->currentTemperatureC - act->lastDecoupleTemp < DECOUPLING_TEST_MIN_TEMP_RISE)   // failed test
                {
                    ...
                    {
                        Printer::setAnyTempsensorDefect();
                        UI_ERROR_P(Messages::tHeaterDecoupled);
                        Console::printErrorFLN(Messages::tHeaterDecoupledWarning);
                        Console::printF(PSTR("Error:Temp. raised to slow. Rise = "),act->currentTemperatureC - act->lastDecoupleTemp);
                        Console::printF(PSTR(" after "),(int32_t)(time-act->lastDecoupleTest));
                        Console::printFLN(PSTR(" ms"));
                    }
                }
                else
                {
            ...
            else     // hold
            {
                if(fabs(act->currentTemperatureC - act->targetTemperatureC) > DECOUPLING_TEST_MAX_HOLD_VARIANCE)   // failed test
                {
                    ...
                    if(extruderTempErrors > 10)   // Ignore short temporary failures
                    {
                        ...
                        UI_ERROR_P(Messages::tHeaterDecoupled);
                        Console::printErrorFLN(Messages::tHeaterDecoupledWarning);
                        Console::printF(PSTR("Error:Could not hold temperature "),act->lastDecoupleTemp);
                        Console::printF(PSTR(" measured "),act->currentTemperatureC);
                        Console::printFLN(PSTR(" deg. C"));
                    }
                }
                else
                {
...
void createGenericTable(short table[GENERIC_THERM_NUM_ENTRIES][2],short minTemp,short maxTemp,float beta,float r0,float t0,float r1,float r2)
{
    ...
    for(uint8_t i=0; i<GENERIC_THERM_NUM_ENTRIES; i++)
    {
        ...
        table[i][1] = (int)t;
#ifdef DEBUG_GENERIC
        Console::printF(Messages::tGenTemp,table[i][0]);
        Console::printFLN(Messages::tComma,table[i][1]);
#endif
    }
}
#endif
...
void Extruder::setTemperatureForExtruder(float temperatureInCelsius,uint8_t extr,bool beep)
{
    ...
    Console::printF(Messages::tTargetExtr,extr,0);
    Console::printFLN(Messages::tColon,temperatureInCelsius,0);
#if FEATURE_DITTO_PRINTING
...
void Extruder::setHeatedBedTemperature(float temperatureInCelsius,bool beep)
{
    ...
    Console::printFLN(Messages::tTargetBedColon,heatedBedController.targetTemperatureC,0);
#endif
}
...
void TemperatureController::autotunePID(float temp,uint8_t controllerId,bool storeValues)
{
    ...
    Console::printInfoFLN(Messages::tPIDAutotuneStart);
    ...
    for(;;)
    {
        ...
        if(heating == false && currentTemp < temp)
        {
            if(time - t1 > (controllerId < NUM_EXTRUDER ? 5000 : 3000))
            {
                ...
                if(cycles > 0)
                {
                    ...
                    Console::printF(Messages::tAPIDBias,bias);
                    Console::printF(Messages::tAPIDD,d);
                    Console::printF(Messages::tAPIDMin,minTemp);
                    Console::printFLN(Messages::tAPIDMax,maxTemp);
                    if(cycles > 2)
                    {
                        ...
                        Console::printF(Messages::tAPIDKu,Ku);
                        Console::printFLN(Messages::tAPIDTu,Tu);
                        Kp = 0.6*Ku;
                        Ki = 2*Kp/Tu;
                        Kd = Kp*Tu*0.125;
                        Console::printFLN(Messages::tAPIDClassic);
                        Console::printFLN(Messages::tAPIDKp,Kp);
                        Console::printFLN(Messages::tAPIDKi,Ki);
                        Console::printFLN(Messages::tAPIDKd,Kd);
                        /*
                        Kp = 0.33*Ku;
                        Ki = Kp/Tu;
                        Kd = Kp*Tu/3;
        ...
        {
            Console::printErrorFLN(Messages::tAPIDFailedHigh);
        ...
        {
            Console::printErrorFLN(Messages::tAPIDFailedTimeout);
            Extruder::disableAllHeater();
            return;
        }
        if(cycles > 5)
        {
            Console::printInfoFLN(Messages::tAPIDFinished);
            Extruder::disableAllHeater();
            if(storeValues)
            {
                pidPGain = Kp;
...
void writeMonitor()
{
    Console::printF(Messages::tMTEMPColon,(long)HAL::timeInMilliseconds());
    TemperatureController *act = tempController[manageMonitor];
    Console::printF(Messages::tSpace,act->currentTemperatureC);
    Console::printF(Messages::tSpace,act->targetTemperatureC,0);
    Console::printFLN(Messages::tSpace,pwm_pos[act->pwmIndex]);
}
...
bool reportTempsensorError()
{
    ...
    for(uint8_t i = 0; i<NUM_TEMPERATURE_LOOPS; i++)
    {
        if(i == NUM_EXTRUDER) Console::printF(Messages::tHeatedBed);
        else Console::printF(Messages::tExtruderSpace,i);
        int temp = tempController[i]->currentTemperatureC;
        if(temp < MIN_DEFECT_TEMPERATURE || temp > MAX_DEFECT_TEMPERATURE)
            Console::printFLN(Messages::tTempSensorDefect);
        else
            Console::printFLN(Messages::tTempSensorWorking);
    }
    Console::printErrorFLN(Messages::tDryModeUntilRestart);
    return true;
}
