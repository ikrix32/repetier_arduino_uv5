...
uint8_t Printer::moveTo(float x,float y,float z,float e,float f)
{
    ...
    {
        Console::printWarningFLN(PSTR("moveTo / queueDeltaMove returns error"));
        return 0;
    }
#else
...
uint8_t Printer::moveToReal(float x, float y, float z, float e, float f)
{
    ...
    {
        Console::printWarningFLN(PSTR("moveToReal / queueDeltaMove returns error"));
        SHOWM(x);
        SHOWM(y);
        SHOWM(z);
        return 0;
    ...
    if(com->hasE() && !Printer::debugDryrun())
    {
        ...
        else
        {
            if(
                ...
                (Extruder::current->tempControl.currentTemperatureC < MIN_EXTRUDER_TEMP  && !Printer::isColdExtrusionAllowed()) ||
#endif
                fabs((float)(p - currentPositionSteps[E_AXIS])) > EXTRUDE_MAXLENGTH * axisStepsPerMM[E_AXIS])
...
#error You have defined hardware z max endstop without pin assignment. Set pin number for Z_MAX_PIN
    ...
    Console::printFLN(Messages::tStart);
    UI_INITIALIZE;
    HAL::showStartReason();
...
void Printer::homeXAxis()
{
    ...
    {
        Console::printWarningFLN(PSTR("homeXAxis / queueDeltaMove returns error"));
    }
}
void Printer::homeYAxis()
{
    ...
    if (!PrintLine::queueDeltaMove(true,false,false))
    {
        Console::printWarningFLN(PSTR("homeYAxis / queueDeltaMove returns error"));
    }
}
void Printer::homeZAxis() // Delta z homing
{
    ...
    //Console::printFLN(Messages::tTower1,dx);
    //Console::printFLN(Messages::tTower2,dy);
    //Console::printFLN(Messages::tTower3,dz);
...
void Printer::homeAxis(bool xaxis,bool yaxis,bool zaxis) // Delta homing code
{
    ...
    {
        Console::printErrorFLN(PSTR("Hardware setup inconsistent. Delta cannot home wihtout max endstops."));
    }
    ...
    else
    {
        ...
        if (!PrintLine::queueDeltaMove(true,false,false))
        {
            Console::printWarningFLN(PSTR("homeAxis / queueDeltaMove returns error"));
        }
    }
...
void Printer::setAutolevelActive(bool on)
{
    ...
    if(on)
        Console::printInfoFLN(Messages::tAutolevelEnabled);
    else
        Console::printInfoFLN(Messages::tAutolevelDisabled);
    updateCurrentPosition(false);
...
float Printer::runZMaxProbe()
{
    ...
    {
        Console::printErrorFLN(Messages::tZProbeFailed);
        return -1;
    }
    ...
    Console::printF(Messages::tZProbeMax,distance);
    Console::printF(Messages::tSpaceXColon,realXPosition());
    Console::printFLN(Messages::tSpaceYColon,realYPosition());
    PrintLine::moveRelativeDistanceInSteps(0,0,-probeDepth,0,EEPROM::zProbeSpeed(),true,true);
    return distance;
}
#endif
...
float Printer::runZProbe(bool first,bool last,uint8_t repeat,bool runStartScript)
{
    ...
    distance += zCorr;
#endif
    Console::printF(Messages::tZProbe, distance);
    Console::printF(Messages::tSpaceXColon, realXPosition());
#if DISTORTION_CORRECTION
    Console::printF(Messages::tSpaceYColon, realYPosition());
    Console::printFLN(PSTR(" zCorr:"), zCorr);
#else
    Console::printFLN(Messages::tSpaceYColon, realYPosition());
#endif
...
void Printer::waitForZProbeStart()
{
    ...
    uid.setStatusP(Messages::tHitZProbe);
    uid.refreshPage();
#endif
#ifdef DEBUG_PRINT
    debugWaitLoop = 3;
...
void Printer::resetTransformationMatrix(bool silent)
{
    ...
    if(!silent)
        Console::printInfoFLN(Messages::tAutolevelReset);
}
...
void Printer::buildTransformationMatrix(float h1,float h2,float h3)
{
    ...
    Console::printArrayFLN(Messages::tTransformationMatrix,autolevelTransformation, 9, 6);
}
#endif
...
void Distortion::init() {
    ...
    Console::printFLN(PSTR("zDistortionCorrection:"),(int)enabled);
#else
    enabled = false;
#endif
}
...
#endif
    Console::printFLN(Messages::tZCorrectionEnabled);
}
...
void Distortion::disable(bool permanent)
{
    ...
    if(permanent)
        EEPROM::setZCorrectionEnabled(enabled);
#endif
    Console::printFLN(Messages::tZCorrectionDisabled);
}

void Distortion::reportStatus() {
    Console::printFLN(enabled ? Messages::tZCorrectionEnabled : Messages::tZCorrectionDisabled);
}
...
void Distortion::measure(void)
{
    ...
    disable(false);
    //Console::printFLN(PSTR("radiusCorr:"), radiusCorrectionSteps);
    //Console::printFLN(PSTR("steps:"), step);
    for (iy = DISTORTION_CORRECTION_POINTS - 1; iy >= 0; iy--)
        for (ix = 0; ix < DISTORTION_CORRECTION_POINTS; ix++)
        {
            ...
            //Console::printF(PSTR("mx "),mtx);
            //Console::printF(PSTR("my "),mty);
            //Console::printF(PSTR("ix "),(int)ix);
            //Console::printFLN(PSTR("iy "),(int)iy);
...
#endif
// print matrix
    Console::printInfoFLN(PSTR("Distortion correction matrix:"));
    for (iy = DISTORTION_CORRECTION_POINTS - 1; iy >=0 ; iy--)
    {
        for(ix = 0; ix < DISTORTION_CORRECTION_POINTS; ix++)
            Console::printF(ix ? PSTR(", ") : PSTR(""), getMatrix(matrixIndex(ix,iy)));
        Console::println();
    }
    enable(true);
...
int32_t Distortion::correct(int32_t x, int32_t y, int32_t z) const
{
    ...
    if(false && z == 0) {
  Console::printF(PSTR("correcting ("), x); Console::printF(PSTR(","), y);
    }
...
// position between cells of matrix, range=0 to 1 - outside of the matrix the value will be outside this range and the value will be extrapolated
    ...
    if(false && z == 0) {
      Console::printF(PSTR(") by "), correction_z);
      Console::printF(PSTR(" ix= "), fxFloor); Console::printF(PSTR(" fx= "), fx);
      Console::printF(PSTR(" iy= "), fyFloor); Console::printFLN(PSTR(" fy= "), fy);
    }
    if (z > zStart && z > 0)
        correction_z *= (zEnd - z) / (zEnd - zStart);
   /* if(correction_z > 20 || correction_z < -20) {
            Console::printFLN(PSTR("Corr. error too big:"),correction_z);
        Console::printF(PSTR("fxf"),(int)fxFloor);
        Console::printF(PSTR(" fyf"),(int)fyFloor);
        Console::printF(PSTR(" fx"),fx);
        Console::printF(PSTR(" fy"),fy);
        Console::printF(PSTR(" x"),x);
        Console::printFLN(PSTR(" y"),y);
        Console::printF(PSTR(" m11:"),m11);
        Console::printF(PSTR(" m12:"),m12);
        Console::printF(PSTR(" m21:"),m21);
        Console::printF(PSTR(" m22:"),m22);
        Console::printFLN(PSTR(" step:"),step);
        correction_z = 0;
    }*/
    return correction_z;
}
