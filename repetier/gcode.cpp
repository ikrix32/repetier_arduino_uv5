...
void GCode::requestResend()
{
    ...
    if(sendAsBinary)
        ...
        waitingForResend = 14;
    Console::println();
    Console::printFLN(Messages::tResend,lastLineNumber+1);
    Console::printFLN(Messages::tOk);
}
...
void GCode::checkAndPushCommand()
{
    ...
    if(hasN())
    {
        if((((lastLineNumber + 1) & 0xffff) != (actLineNumber&0xffff)))
        {
            if(static_cast<uint16_t>(lastLineNumber - actLineNumber) < 40)
            {
                ...
                commandsReceivingWritePosition = 0;
                Console::printFLN(Messages::tSkip,actLineNumber);
                Console::printFLN(Messages::tOk);
            }
            else if(waitingForResend < 0)  // after a resend, we have to skip the garbage in buffers, no message for this
            {
                ...
                {
                    Console::printF(Messages::tExpectedLine, lastLineNumber + 1);
                    Console::printFLN(Messages::tGot, actLineNumber);
                }
    ...
    Console::printFLN(Messages::tOkSpace, actLineNumber);
#else
    Console::printFLN(Messages::tOk);
#endif
...
{
    Console::printF(PSTR("CommandBuffer"));
    for(int i = 0; i < commandsReceivingWritePosition; i++)
        Console::printF(Messages::tColon,(int)commandReceiving[i]);
    Console::println();
    Console::printFLN(PSTR("Binary:"), (int)sendAsBinary);
    if(!sendAsBinary)
    {
        Console::print((char*)commandReceiving);
        Console::println();
    }
}
...
void GCode::readFromSerial()
{
    ...
    if(!HAL::serialByteAvailable())
    {
        ...
        {
            Console::printFLN(Messages::tWait); // Unblock communication in case the last ok was not received correct.
            timeOfLastDataPacket = time;
        }
#endif
    }
    while(HAL::serialByteAvailable() && commandsReceivingWritePosition < MAX_CMD_SIZE)    // consume data until no data or buffer full
    {
        ...
        else     // Ascii command
        {
            ...
            {
                //Console::printF(PSTR("Parse ascii"));Console::print((char*)commandReceiving);Console::println();
    ...
    while( sd.filesize > sd.sdpos && commandsReceivingWritePosition < MAX_CMD_SIZE)    // consume data until no data or buffer full
    {
        ...
        if(n == -1)
        {
            Console::printFLN(Messages::tSDReadError);
            ...
            if(n == -1)
            {
                Console::printErrorFLN(PSTR("SD error did not recover!"));
                sd.sdmode = 0;
                break;
            }
    ...
    sd.sdmode = 0;
    Console::printFLN(Messages::tDonePrinting);
    commandsReceivingWritePosition = 0;
    commentDetected = false;
...
bool GCode::parseBinary(uint8_t *buffer,bool fromSerial)
{
    ...
    if(sum1 | sum2)
    {
        if(Printer::debugErrors())
        {
            Console::printErrorFLN(Messages::tWrongChecksum);
        }
        return false;
    }
    p = buffer;
...
bool GCode::parseAscii(char *line,bool fromSerial)
{
    ...
    if(hasFormatError() || (params & 518)==0)   // Must contain G, M or T command and parameter need to have variables!
    {
        formatErrors++;
        if(Printer::debugErrors())
            Console::printErrorFLN(Messages::tFormatError);
...
void GCode::printCommand()
{
    ...
    if(hasX())
    {
        Console::printF(Messages::tX,X);
    }
    if(hasY())
    {
        Console::printF(Messages::tY,Y);
    }
    if(hasZ())
    {
        Console::printF(Messages::tZ,Z);
    }
    if(hasE())
    {
        Console::printF(Messages::tE,E,4);
    }
    if(hasF())
    {
        Console::printF(Messages::tF,F);
    }
    if(hasS())
    {
        Console::printF(Messages::tS,S);
    }
    if(hasP())
    {
        Console::printF(Messages::tP,P);
    }
    if(hasI())
    {
        Console::printF(Messages::tI,I);
    }
    if(hasJ())
    {
        Console::printF(Messages::tJ,J);
    }
    if(hasR())
    {
        Console::printF(Messages::tR,R);
    }
    if(hasString())
    {
        Console::print(text);
