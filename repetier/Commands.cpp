...
void Commands::printCurrentPosition(FSTRINGPARAM(s))
{
    ...
    {
        Console::printErrorFLN(s); // flag where the error condition came from
    }
    x += Printer::coordinateOffset[X_AXIS];
    y += Printer::coordinateOffset[Y_AXIS];
    z += Printer::coordinateOffset[Z_AXIS];
    Console::printF(Messages::tXColon, x * (Printer::unitIsInches ? 0.03937 : 1), 2);
    Console::printF(Messages::tSpaceYColon, y * (Printer::unitIsInches ? 0.03937 : 1), 2);
    Console::printF(Messages::tSpaceZColon, z * (Printer::unitIsInches ? 0.03937 : 1), 3);
    Console::printFLN(Messages::tSpaceEColon, Printer::currentPositionSteps[E_AXIS] * Printer::invAxisStepsPerMM[E_AXIS] * (Printer::unitIsInches ? 0.03937 : 1), 4);
    //Console::printF(PSTR("OffX:"),Printer::offsetX); // to debug offset handling
    //Console::printFLN(PSTR(" OffY:"),Printer::offsetY);
}
...
void Commands::printTemperatures(bool showRaw)
{
    ...
    Console::printF(Messages::tTColon,temp);
    Console::printF(Messages::tSpaceSlash,Extruder::current->tempControl.targetTemperatureC,0);
#else
    Console::printF(Messages::tTColon,temp);
    Console::printF(Messages::tSpaceSlash,Extruder::current->tempControl.targetTemperatureC,0);
#if HAVE_HEATED_BED
    Console::printF(Messages::tSpaceBColon,Extruder::getHeatedBedTemperature());
    Console::printF(Messages::tSpaceSlash,heatedBedController.targetTemperatureC,0);
    if(showRaw)
    {
        Console::printF(Messages::tSpaceRaw,(int)NUM_EXTRUDER);
        Console::printF(Messages::tColon,(1023<<(2-ANALOG_REDUCE_BITS))-heatedBedController.currentTemperature);
    }
    Console::printF(Messages::tSpaceBAtColon,(pwm_pos[heatedBedController.pwmIndex])); // Show output of autotune when tuning!
#endif
#endif
#if TEMP_PID
    Console::printF(Messages::tSpaceAtColon,(int)(autotuneIndex==255?pwm_pos[Extruder::current->id]:pwm_pos[autotuneIndex])); // Show output of autotune when tuning!
#endif
#if NUM_EXTRUDER>1 && MIXING_EXTRUDER == 0
    for(uint8_t i=0; i<NUM_EXTRUDER; i++)
    {
        Console::printF(Messages::tSpaceT,(int)i);
        Console::printF(Messages::tColon,extruder[i].tempControl.currentTemperatureC);
        Console::printF(Messages::tSpaceSlash,extruder[i].tempControl.targetTemperatureC,0);
#if TEMP_PID
        Console::printF(Messages::tSpaceAt,(int)i);
        Console::printF(Messages::tColon,(pwm_pos[extruder[i].tempControl.pwmIndex])); // Show output of autotune when tuning!
#endif
        if(showRaw)
        {
            Console::printF(Messages::tSpaceRaw,(int)i);
            Console::printF(Messages::tColon,(1023<<(2-ANALOG_REDUCE_BITS))-extruder[i].tempControl.currentTemperature);
        }
    }
#endif
    Console::println();
...
void Commands::changeFeedrateMultiply(int factor)
{
    ...
    Console::printFLN(Messages::tSpeedMultiply, factor);
}
...
void Commands::changeFlowateMultiply(int factor)
{
    ...
    Console::printFLN(Messages::tFlowMultiply, factor);
}
...
void Commands::setFanSpeed(int speed,bool wait)
{
    ...
    if(speed != pwm_pos[NUM_EXTRUDER + 2])
        Console::printFLN(Messages::tFanspeed,speed); // send only new values to break update loops!
...
void Commands::reportPrinterUsage()
{
    ...
    Console::printF(Messages::tPrintedFilament,dist,2);
    Console::printF(Messages::tSpacem);
    bool alloff = true;
    for(uint8_t i=0; i<NUM_EXTRUDER; i++)
    ...
    seconds -= tmp * 86400;
    Console::printF(Messages::tPrintingTime,tmp);
    tmp=seconds / 3600;
    Console::printF(Messages::tSpaceDaysSpace,tmp);
    seconds-=tmp * 3600;
    tmp = seconds / 60;
    Console::printF(Messages::tSpaceHoursSpace,tmp);
    Console::printFLN(Messages::tSpaceMin);
#endif
}
...
{
    Console::printFLN(Messages::tMS1MS2Pins);
    Console::printF(Messages::tXColon,READ(X_MS1_PIN));
    Console::printFLN(Messages::tComma,READ(X_MS2_PIN));
    Console::printF(Messages::tYColon,READ(Y_MS1_PIN));
    Console::printFLN(Messages::tComma,READ(Y_MS2_PIN));
    Console::printF(Messages::tZColon,READ(Z_MS1_PIN));
    Console::printFLN(Messages::tComma,READ(Z_MS2_PIN));
    Console::printF(Messages::tE0Colon,READ(E0_MS1_PIN));
    Console::printFLN(Messages::tComma,READ(E0_MS2_PIN));
    Console::printF(Messages::tE1Colon,READ(E1_MS1_PIN));
    Console::printFLN(Messages::tComma,READ(E1_MS2_PIN));
}
#endif
...
void Commands::processArc(GCode *com)
{
    ...
    if (com->hasR())
    {
        ...
        */
        ...
        if(isnan(h_x2_div_d))
        {
            Console::printErrorFLN(Messages::tInvalidArc);
            return;
        }
...
void Commands::processGCode(GCode *com)
{
    ...
    case 1: // G1
        ...
        if(Printer::setDestinationStepsFromGCode(com)) // For X Y Z E F
            ...
            if (!PrintLine::queueDeltaMove(ALWAYS_CHECK_ENDSTOPS, true, true)) {
                Console::printWarningFLN(PSTR("executeGCode / queueDeltaMove returns error"));
            }
#else
    ...
    case 29: // G29 3 points, build average or distortion compensation
    {
...
#else
        GCode::executeFString(Messages::tZProbeStartScript);
        ...
        Console::printFLN(Messages::tZProbeAverage,sum);
        if(com->hasS() && com->S)
        {
...
#endif
            Console::printInfoFLN(Messages::tZProbeZReset);
            Console::printFLN(Messages::tZProbePrinterHeight,Printer::zLength);
#else
            Printer::currentPositionSteps[Z_AXIS] = sum * Printer::axisStepsPerMM[Z_AXIS];
            Console::printFLN(PSTR("Adjusted z origin"));
#endif
        }
        Printer::feedrate = oldFeedrate;
        ...
        GCode::executeFString(Messages::tZProbeEndScript);
        Printer::feedrate = oldFeedrate;
    ...
    case 31:  // G31 display hall sensor output
        Console::printF(Messages::tZProbeState);
        Console::print(Printer::isZProbeHit() ? 'H' : 'L');
        Console::println();
        break;
    ...
    case 90: // G90
        ...
        if(com->internalCommand)
            Console::printInfoFLN(PSTR("Absolute positioning"));
        break;
    case 91: // G91
        Printer::relativeCoordinateMode = true;
        if(com->internalCommand)
            Console::printInfoFLN(PSTR("Relative positioning"));
        break;
    case 92: // G92
    {
    ...
    case 100: // G100 Calibrate floor or rod radius
    {
        ...
        {
            Console::printErrorFLN(PSTR("Calibration code is limited to bottom 10% of Z height"));
            break;
        }
        if (com->hasR())
        {
            if (com->hasX() || com->hasY() || com->hasZ())
                Console::printErrorFLN(PSTR("Cannot set radius and floor at same time."));
            else if (com->R != 0)
            {
                //add r to radius
                if (abs(com->R) <= 10) EEPROM::incrementRodRadius(com->R);
                else Console::printErrorFLN(PSTR("Calibration movement is limited to 10mm."));
            }
            else
            {
                        ...
                        && Printer::currentPosition[Y_AXIS] == 0)
                {
                    ...
                    Console::printErrorFLN(PSTR("First move to touch at x,y=0,0 to auto-set radius."));
            }
        }
        else
        {
            ...
            if (tooBig)
                Console::printErrorFLN(PSTR("Calibration movement is limited to 10mm."));
        }
...
// but you will always get different counts from different positions.
        ...
        Console::printFLN(Messages::tTower1,offx);
        Console::printFLN(Messages::tTower2,offy);
        Console::printFLN(Messages::tTower3,offz);
#if EEPROM_MODE != 0
        if(com->hasS() && com->S>0)
        {
...
// so it is unclear how this is helpful. It must start at a well defined point.
    ...
    break;
    case 134: // G134
        Console::printF(PSTR("CompDelta:"),Printer::currentDeltaPositionSteps[A_TOWER]);
        Console::printF(Messages::tComma,Printer::currentDeltaPositionSteps[B_TOWER]);
        Console::printFLN(Messages::tComma,Printer::currentDeltaPositionSteps[C_TOWER]);
#ifdef DEBUG_REAL_POSITION
        Console::printF(PSTR("RealDelta:"),Printer::realDeltaPositionSteps[A_TOWER]);
        Console::printF(Messages::tComma,Printer::realDeltaPositionSteps[B_TOWER]);
        Console::printFLN(Messages::tComma,Printer::realDeltaPositionSteps[C_TOWER]);
#endif
        Printer::updateCurrentPosition();
        Console::printF(PSTR("PosFromSteps:"));
    ...
    default:
        ...
        {
            Console::printF(Messages::tUnknownCommand);
            com->printCommand();
        }
    }
...
void Commands::processMCode(GCode *com)
{
    ...
    case 42: //M42 -Change pin status via gcode
        if (com->hasS() && com->hasP() && com->S>=0 && com->S<=255)
        {
            ...
            if (pin_number > -1)
            {
                ...
                Console::printF(Messages::tSetOutputSpace,pin_number);
                Console::printFLN(Messages::tSpaceToSpace,(int)com->S);
            }
        }
        break;
    ...
    case 111: // M111 enable/disable run time debug flags
        ...
        break;
    case 115: // M115
        Console::printFLN(Messages::tFirmware);
        reportPrinterUsage();
        break;
    case 114: // M114
    ...
    case 119: // M119
        ...
        Console::printF(Messages::tXMinColon);
        Console::printF(Printer::isXMinEndstopHit()?Messages::tHSpace:Messages::tLSpace);
#endif
#if (X_MAX_PIN > -1) && MAX_HARDWARE_ENDSTOP_X
        Console::printF(Messages::tXMaxColon);
        Console::printF(Printer::isXMaxEndstopHit()?Messages::tHSpace:Messages::tLSpace);
#endif
#if (Y_MIN_PIN > -1) && MIN_HARDWARE_ENDSTOP_Y
        Console::printF(Messages::tYMinColon);
        Console::printF(Printer::isYMinEndstopHit()?Messages::tHSpace:Messages::tLSpace);
#endif
#if (Y_MAX_PIN > -1) && MAX_HARDWARE_ENDSTOP_Y
        Console::printF(Messages::tYMaxColon);
        Console::printF(Printer::isYMaxEndstopHit()?Messages::tHSpace:Messages::tLSpace);
#endif
#if (Z_MIN_PIN > -1) && MIN_HARDWARE_ENDSTOP_Z
        Console::printF(Messages::tZMinColon);
        Console::printF(Printer::isZMinEndstopHit()?Messages::tHSpace:Messages::tLSpace);
#endif
#if (Z_MAX_PIN > -1) && MAX_HARDWARE_ENDSTOP_Z
        Console::printF(Messages::tZMaxColon);
        Console::printF(Printer::isZMaxEndstopHit()?Messages::tHSpace:Messages::tLSpace);
#endif
        Console::println();
        break;
#if BEEPER_TYPE>0
    ...
    case 207: // M207 X<XY jerk> Z<Z Jerk>
        ...
        Console::printF(Messages::tJerkColon,Printer::maxJerk);
        Console::printFLN(Messages::tZJerkColon,Printer::maxZJerk);
#else
        Console::printFLN(Messages::tJerkColon,Printer::maxJerk);
#endif
        break;
    ...
    case 223: // M223 Extruder interrupt test
        ...
        break;
    case 232: // M232
        Console::printF(Messages::tLinearStepsColon,maxadv2);
#if ENABLE_QUADRATIC_ADVANCE
        Console::printF(Messages::tQuadraticStepsColon,maxadv);
#endif
        Console::printFLN(Messages::tCommaSpeedEqual,maxadvspeed);
#if ENABLE_QUADRATIC_ADVANCE
        maxadv=0;
#endif
        maxadv2=0;
    ...
    case 233: // M233
        ...
        Console::printF(Messages::tLinearLColon,Extruder::current->advanceL);
#if ENABLE_QUADRATIC_ADVANCE
        if(com->hasX())
            Extruder::current->advanceK = com->X;
        Console::printF(Messages::tQuadraticKColon,Extruder::current->advanceK);
#endif
        Console::println();
    ...
    case 251: // M251
        ...
        Console::printFLN(Messages::tEEPROMUpdated);
#endif
...
#if FEATURE_WATCHDOG
    {
        Console::printInfoFLN(PSTR("Triggering watchdog. If activated, the printer will reset."));
        Printer::kill(false);
...
#else
    Console::printInfoFLN(PSTR("Watchdog feature was not compiled into this version!"));
#endif
    break;
...
#if EEPROM_MODE != 0
        EEPROM::storeDataIntoEEPROM(false);
        Console::printInfoF(Messages::tConfigStoredEEPROM);
#else
    ...
    case 501: // M501
    {
        ...
        Extruder::selectExtruderById(Extruder::current->id);
        Console::printInfoF(Messages::tConfigLoadedEEPROM);
#else
        Console::printErrorF(Messages::tNoEEPROMSupport);
#endif
    }
    ...
    case 533: // M533 Write move data
        Console::printF(PSTR("Buf:"),(int)PrintLine::linesCount);
        Console::printF(PSTR(",LP:"),(int)PrintLine::linesPos);
        Console::printFLN(PSTR(",WP:"),(int)PrintLine::linesWritePos);
        if(PrintLine::cur == NULL)
        {
            Console::printFLN(PSTR("No move"));
            if(PrintLine::linesCount>0)
            {
                PrintLine &cur = PrintLine::lines[PrintLine::linesPos];
                Console::printF(PSTR("JFlags:"),(int)cur.joinFlags);
                Console::printFLN(PSTR("Flags:"),(int)cur.flags);
                if(cur.isWarmUp())
                {
                    Console::printFLN(PSTR("warmup:"),(int)cur.getWaitForXLinesFilled());
                }
            }
        }
        else
        {
            Console::printF(PSTR("Rem:"),PrintLine::cur->stepsRemaining);
            Console::printFLN(PSTR("Int:"),Printer::interval);
        }
        break;
#endif // DEBUG_QUEUE_MOVE
#ifdef DEBUG_SEGMENT_LENGTH
    case 534: // M534
        Console::printFLN(PSTR("Max. segment size:"),Printer::maxRealSegmentLength);
        if(com->hasS())
            Printer::maxRealSegmentLength = 0;
        break;
#endif
#ifdef DEBUG_REAL_JERK
        Console::printFLN(PSTR("Max. jerk measured:"),Printer::maxRealJerk);
        if(com->hasS())
            Printer::maxRealJerk = 0;
        break;
#endif
        /*      case 535:  // M535
                    Console::printF(PSTR("Last commanded position:"),Printer::lastCmdPos[X_AXIS]);
                    Console::printF(Messages::tComma,Printer::lastCmdPos[Y_AXIS]);
                    Console::printFLN(Messages::tComma,Printer::lastCmdPos[Z_AXIS]);
                    Console::printF(PSTR("Current position:"),Printer::currentPosition[X_AXIS]);
                    Console::printF(Messages::tComma,Printer::currentPosition[Y_AXIS]);
                    Console::printFLN(Messages::tComma,Printer::currentPosition[Z_AXIS]);
                    Console::printF(PSTR("Position steps:"),Printer::currentPositionSteps[X_AXIS]);
                    Console::printF(Messages::tComma,Printer::currentPositionSteps[Y_AXIS]);
                    Console::printFLN(Messages::tComma,Printer::currentPositionSteps[Z_AXIS]);
        #if NONLINEAR_SYSTEM
              Console::printF(PSTR("Nonlin. position steps:"),Printer::currentDeltaPositionSteps[A_TOWER]);
              Console::printF(Messages::tComma,Printer::currentDeltaPositionSteps[B_TOWER]);
              Console::printFLN(Messages::tComma,Printer::currentDeltaPositionSteps[C_TOWER]);
        ...
        /* case 700: // M700 test new square root function
              if(com->hasS())
                  Console::printFLN(Messages::tInfo,(int32_t)HAL::integerSqrt(com->S));
              break;*/
...
void Commands::writeLowestFreeRAM()
{
    if(lowestRAMValueSend>lowestRAMValue)
    {
        ...
        Console::printFLN(Messages::tFreeRAM, lowestRAMValue);
    }
}
